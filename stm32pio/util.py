...
def generate_code(dirty_path):
    ...
    cubemx_script_full_filename = project_path.joinpath(stm32pio.settings.cubemx_script_filename)
    if not cubemx_script_full_filename.is_file():
        ...
        cubemx_script_content = stm32pio.settings.cubemx_script_content.format(
            project_path=project_path, cubemx_ioc_full_filename=cubemx_ioc_full_filename)
...
def pio_init(dirty_path, board):
    ...
    command_arr = [stm32pio.settings.platformio_cmd, 'init', '-d', str(project_path), '-b', board,
...
def pio_build(dirty_path):
    ...
    command_arr = [stm32pio.settings.platformio_cmd, 'run', '-d', str(project_path)]
    if logger.level > logging.DEBUG:
        command_arr.append('--silent')
    result = subprocess.run(command_arr)
    if result.returncode == 0:
